...
public:
    UtilsKeybordManager():mCurKey(mcDefaultKey), mThreadOn(false){};
...
bool UtilsKeybordManager::start(){
    if(mKeybordManagerThread){  //只能启动并允许使用一次!
        MYLOG_FW("Warn, start too many times is not allowed");
        return false;
    }
    mThreadOn = true;
...
void UtilsKeybordManager::keybordReaderFun(std::weak_ptr<UtilsKeybordManager> keybordManager){
    ...
    while(keybordManager.lock() && keybordManager.lock()->mThreadOn){
        ...
        MYLOG_FW("read key from bord, key=," + std::string(1, keybordManager.lock()->mCurKey) + ".");
        usleep(1000 * 5);
    }
