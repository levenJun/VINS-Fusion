...
for (int cid = 0; cid < NUM_CAM; cid++){
    ...
    cv::Mat rightImg = vTrackInfoMono[1].cur_img;
    ...
    for (int cid = 0; cid < NUM_CAM; cid++){
        ...
        vTrackInfoMono[cid].cur_un_right_pts = undistortedPts(vTrackInfoMono[cid].cur_right_pts, m_camera[1]);
...
for (int cid = 0; cid < NUM_CAM; cid++)
{
    ...
    if (!vTrackInfoMono[cid].cur_img.empty() && stereo_cam)
    {
