        ...
        */
        ...
        if (vTrackInfoMono[cid].prev_pts.size() > 0)
        {
            ...
            if(hasPrediction[cid])
            {
                ...
                cv::calcOpticalFlowPyrLK(vTrackInfoMono[cid].prev_img, vTrackInfoMono[cid].cur_img, vTrackInfoMono[cid].prev_pts, vTrackInfoMono[cid].cur_pts, status, err, cv::Size(21, 21), 2);
            }
            else
                // cv::calcOpticalFlowPyrLK(vTrackInfoMono[cid].prev_img, vTrackInfoMono[cid].cur_img, vTrackInfoMono[cid].prev_pts, vTrackInfoMono[cid].cur_pts, status, err, cv::Size(21, 21), 3);
                cv::calcOpticalFlowPyrLK(vTrackInfoMono[cid].prev_img, vTrackInfoMono[cid].cur_img, vTrackInfoMono[cid].prev_pts, vTrackInfoMono[cid].cur_pts, status, err, cv::Size(21, 21), 2);
    ...
    for (int ldx = 0; ldx < 1; ldx++)
    {
        ...
        if(!rightImg.empty() && stereo_cam)
        {
            ...
            if(!vTrackInfoMono[cid].cur_pts.empty())
            {
                ...
                cv::calcOpticalFlowPyrLK(vTrackInfoMono[cid].cur_img, rightImg, vTrackInfoMono[cid].cur_pts, vTrackInfoMono[cid].cur_right_pts, status, err, cv::Size(21, 21), 1, 
                ...
                if(FLOW_BACK)
                {
                    ...
                    cv::calcOpticalFlowPyrLK(rightImg, vTrackInfoMono[cid].cur_img, vTrackInfoMono[cid].cur_right_pts, reverseLeftPts, statusRightLeft, err, cv::Size(21, 21), 1, 
                                            cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 30, 0.01));
                    for(size_t i = 0; i < status.size(); i++)
                    {
